  ...
  socket.on('join_auction', (auction_id) => {
    ...
    if (auctionTimers[auction_id]) {
      socket.emit('auction_sync', {
        ...
        bidHistory: auctionTimers[auction_id].bidHistory || []
      });
    }
  });

  const updateBidHistory = (auction_id, bid) => {
    if (!auctionTimers[auction_id].bidHistory) {
      auctionTimers[auction_id].bidHistory = [];
    }
    // Add to front and keep last 5
    auctionTimers[auction_id].bidHistory = [bid, ...auctionTimers[auction_id].bidHistory].slice(0, 5);
  };
  ...
  socket.on('place_bid', async (data) => {
    try {
      ...
      if (auctionTimers[auction_id]) {
        ...
        updateBidHistory(auction_id, { team_name, amount, team_logo, team_id });
      }
